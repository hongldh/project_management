...
from common.models import ProjectEquipmentSchedule, ScheduleHistory
...
from common.models import ProjectBasicInfo
...
def schedule_apply(request):
    projects = ProjectBasicInfo.objects.all().values('project_id', 'project_name')
...
def schedule_apply_project(request, project_id):
    ...
    if request.method == 'POST':
        ...
        for key in request.POST:
            if key.startswith('start_'):
                ...
                try:
                    schedule = ProjectEquipmentSchedule.objects.get(
                ...
                except ProjectEquipmentSchedule.DoesNotExist:
    ...
    # 获取项目基础信息
    project = ProjectBasicInfo.objects.get(project_id=project_id)
    ...
    # 获取排程数据并按指定顺序排序
    schedules = ProjectEquipmentSchedule.objects.filter(project_id=project_id).extra(
        select={
    ...
    # 获取历史记录
    history = ScheduleHistory.objects.filter(project_id=project_id).order_by('-modified_at')
    ...
    #               ,pes.start_time
    #               ,pes.end_time
    #           FROM common_projectequipmentschedule pes
    #           LEFT JOIN common_projectbasicinfo pbi
    ...
    #                   FROM common_schedulehistory sh
    #                   LEFT JOIN common_projectbasicinfo pbi
