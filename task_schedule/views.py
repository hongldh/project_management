...
def task_schedule_test(request):
...
def task_schedule(request):
        ...
        for key in request.POST:
            if key.startswith('start_'):
                ...
                try:
                    ...
                    # print("schedule:")
                    # print(schedule)
                    # print(schedule.equipment_quantity)
    ...
    # schedules = ProjectEquipmentSchedule.objects.all().order_by('project_id', 'equipment_id')
    # 执行原生SQL查询
    ...
    # history = ScheduleHistory.objects.all().order_by('-modified_at')
    with connection.cursor() as cursor:
        history_sql = """
                      SELECT
                           sh.*
    ...
    return render(request, 'task_schedule/task_schedule.html', {
         'schedules': schedules,
         'history': history
    })